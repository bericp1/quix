...
var reenableQuestions = function(client, res, done, cb){
  var body = {};
  cb = cb || function(){};
...
var newQuestion = function(client, d, res, cb, remedy){
  cb = cb || function () {};
  var done = function () {
    d();
    cb();
                  ...
                  function(disErr, disResult){
                    if(disErr){
                      ...
                      if(disResult.rowCount < 1){
                        if(remedy){
                          reenableQuestions(client, {send:function(){}}, function(){}, function(){
                            newQuestion(client, d, res, cb, false);
                          });
                        }else{
                          body.ok = false;
...
app.get('/new_question', function (req, res, next) {
  ...
  pg.connect(pgConInfo, function(err, client, done){
    ...
    newQuestion(client, done, res, function () {});

  });
});

var currentQuestion = function (client, done, res, remedy) {
  var body = {};
  client.query('SELECT * FROM status WHERE name = \'current_question_id\'', function(err, result) {
    if(!err){
      ...
      if(newId == -1){
        ...
        if(remedy){
          newQuestion(client, function(){}, {send:function(){}}, function(){
            currentQuestion(client, done, res, flase);
          });
        }else{
          body.ok = false;
...
app.get('/current_question', function (req, res, next) {
  ...
  pg.connect(pgConInfo, function(err, client, done){
    currentQuestion(client, done, res, true);
  });
});
...
app.get("/admin/reenablequestions", function(req, res, next){
  ...
  if( req.query.password != app.get('adminpass') ){
    ...
    pg.connect(pgConInfo, function(err, client, done){
      reenableQuestions(client, res, done, function(){});
    });
